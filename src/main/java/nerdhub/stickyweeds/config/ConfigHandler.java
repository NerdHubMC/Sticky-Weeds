    ...
    public static <T> T reloadConfig(Class<T> configClass) {
        ...
        if(!configFile.exists()) {
            try(FileWriter writer = new FileWriter(configFile)) {
                ...
                StickyWeeds.logger.error("unable to write config file for {}", configClass::getCanonicalName);
                StickyWeeds.logger.trace("file location: " + configFile.getAbsolutePath(), e);
            }
        }
        T config;
        try(FileReader reader = new FileReader(configFile)) {
            ...
            StickyWeeds.logger.error("unable to read config file from " + configFile.getAbsolutePath() + ", falling back to default values!", e);
            try {
